...
```{r}
missstates <- c("AZ", "CA", "ID", "ME", "MT", "NJ", "NM", "NV", "OR", "RI", "UT", "WA", "DC")
...
```{r, eval = FALSE}
startday = format(as.POSIXct(strptime(BigDays.sfdfT$startTime,"%Y-%m-%d %H:%M:%S",tz="")) ,format = "%Y/%m/%d")
starthour = format(as.POSIXct(strptime(BigDays.sfdfT$startTime,"%Y-%m-%d %H:%M:%S",tz="")) ,format = "%H")

endday = format(as.POSIXct(strptime(BigDays.sfdfT$finishTime,"%Y-%m-%d %H:%M:%S",tz="")) ,format = "%Y/%m/%d")
endhour = format(as.POSIXct(strptime(BigDays.sfdfT$finishTime,"%Y-%m-%d %H:%M:%S",tz="")) ,format = "%H")
...
BigDays.sfdfT <- cbind(BigDays.sfdfT, startday, starthour, endday, endhour, NARRday, NARRZtime )
```
...
```{r}
model1 <- lmer(log(GroupTotalED) ~ (1|Month) + 
                  mBS2 + mCAPE2*mHLCY2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(model1)

model2 <- lmer(log(GroupTotalED) ~ (1|Month) + 
                  mBS2 + mCAPE2 + mHLCY2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(model2)
