...
```{r}
library(lme4)
m1 <- lmer(casualties ~ I(Year - 2004) + I(maxPopDens/100) + (1|Month),
             weights = nT, 
             data = BigDays.sfdfT)
summary(m1)

m2 <- lmer(casualties ~ I(maxPopDens/100) + (1|Month) + maxCAPE2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(m2) # No to average CAPE

m3 <- lmer(casualties ~ I(maxPopDens/100) + (1|Month) + avgBS2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(m3) #No to average 

m4 <- lmer(casualties ~ I(maxPopDens/100) + (1|Month) + maxHLCY2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(m4) # No to average

m5 <- lmer(casualties ~ I(maxPopDens/100) + (1|Month) + avgHLCY2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(m5) # No to average

m6 <- lmer(casualties ~ I(maxPopDens/100) + (1|Month) + avgCIN2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(m6) # No to average

m6 <- lmer(casualties ~ I(maxPopDens/100) + (1|Month) + maxCAPE2 * maxHLCY2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(m6)

m7 <- lmer(casualties ~ I(maxPopDens/100) + (1|Month) + maxCAPE2 * minCIN2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(m7) # No interaction with CAPE and CIN needed

m8 <- lmer(casualties ~ I(maxPopDens/100) + (1|Month) + maxCAPE2 * maxHLCY2 * minCIN2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(m8)
```
...
```{r}
model1 <- lmer(casualties ~ I(maxPopDens/100) + (1|Month) + maxBS2 + maxCAPE2 * maxHLCY2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(model1)

model2 <- lmer(casualties ~ I(maxPopDens/100) + (1|Month) + maxBS2 + maxCAPE2 + maxHLCY2 + minCIN2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(model2)

model3 <- lmer(casualties ~ I(maxPopDens/100) + (1|Month) + maxBS2 + maxCAPE2 * maxHLCY2 + minCIN2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(model3)
...
#14000 HLCY#
############
Extract the centroid, hull, and tornadoes for April 21 (i = 188)
```{r}
