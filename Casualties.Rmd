...
Get the casualty count by state into a single data frame
```{r, eval = FALSE}
casualty <- Tor.sfdfT %>%
  group_by(st) %>%
  summarize(cas = n())
...
for(i in 1:length(slug)){
  print(i)
  rb <- brick(paste0("/Volumes/My Passport for Mac/PhD_Program/NCARNARR/Files/18Z/", df$slug[i]))
...
```
rows 306 and 549 produce these arguments: 
no non-missing arguments to min; returning Infno non-missing arguments to max; returning -Infno non-missing arguments to min; returning Infno non-missing arguments to max; returning -Infno non-missing arguments to min; returning Infno non-missing arguments to max; returning -Infno non-missing arguments to min; returning  
...
dim(df)
```
**What do we do about days that have max and average CAPE 0? ** 4-21-2001 and 5-23-2010  
```{r}
