...
## MODELS FOR ATE ##
####################
Check it variable as a predictor of ATE
```{r}
library(lme4)
...
m5 <- lmer(log(GroupTotalED) ~ I(Year - 2004) + (1|Month) + avgCIN2,
             weights = nT, 
             data = BigDays.sfdfT)
...
m7 <- lmer(log(GroupTotalED) ~ I(Year - 2004) + (1|Month) + maxCAPE2 * minCIN2 * maxHLCY2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(m7) # No interaction with CAPE and CIN needed
```
Fit the best models using the significant predictors. 
```{r}
...
```
Model 1 has the lowest AIC. This is the model with the CAPE and HLCY interaction. 
...
```
** Issue: The May 30, 2003 event has a HLCY value of 14000. The closest helicity value to that big day is 1026. Creates a huge outlier. How do we account for this?**
...
```{r}
library(lme4)
m1 <- lmer(log(casualties+1) ~ I(Year - 2004) + I(avgPopDens/100) + (1|Month),
             weights = nT, 
             data = BigDays.sfdfT)
summary(m1)

m2 <- lmer(log(casualties+1)  ~ I(Year - 2004) + I(maxPopDens/100) + (1|Month) + maxCAPE2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(m2) # No to average CAPE

m3 <- lmer(log(casualties+1)  ~ I(Year - 2004) + I(maxPopDens/100) + (1|Month) + maxBS2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(m3) #No to average 

m4 <- lmer(log(casualties+1)  ~ I(Year - 2004) + I(maxPopDens/100) + (1|Month) + maxHLCY2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(m4) # No to average

m5 <- lmer(casualties ~ I(Year - 2004) + I(maxPopDens/100) + (1|Month) + minCIN2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(m5) # No to average

m6 <- lmer(log(casualties+1)  ~ I(Year - 2004) + I(maxPopDens/100) + (1|Month) + maxCAPE2 * maxHLCY2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(m6)

m7 <- lmer(log(casualties+1)  ~ I(Year - 2004) + I(maxPopDens/100) + (1|Month) + maxCAPE2 * minCIN2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(m7) # No interaction with CAPE and CIN needed

m8 <- lmer(log(casualties+1)  ~ I(Year - 2004) + I(maxPopDens/100) + (1|Month) + maxCAPE2 * maxHLCY2 * minCIN2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(m8)
```
...
```{r}
model1 <- lmer(log(casualties+1)  ~ I(maxPopDens/100) + (1|Month) + maxBS2 + minCIN2 + maxCAPE2 * maxHLCY2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(model1)

model2 <- lmer(log(casualties+1)  ~ I(maxPopDens/100) + (1|Month) + maxBS2 + maxCAPE2 + maxHLCY2 + minCIN2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(model2)

model3 <- lmer(log(casualties+1)  ~ I(maxPopDens/100) + (1|Month) + maxBS2 + maxCAPE2 * maxHLCY2 * minCIN2,
             weights = nT, 
             data = BigDays.sfdfT)
summary(model3)
...
```
** Issue: The May 30, 2003 event has a HLCY value of 14000. The closest value to that big day is 1026. Creates a huge outlier. How do we account for this?**
